/**
 * Aetosky Design System Composable
...
export const useDesignSystem = () => {
  ...
  const getTypographyStyles = (variant, weight = 'normal') => {
  ...
  const getColorWithOpacity = (colorPath, opacity = 1) => {
  ...
  const getSpacing = (key) => spacing[key] || '0px'

  // Helper function to create consistent component styles
  const createComponentStyle = (baseClasses = '', variants = {}) => {
    return {
      base: baseClasses,
      variants,
    }
